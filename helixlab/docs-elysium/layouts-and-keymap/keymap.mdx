...
<Icon icon="fa-solid fa-square" style={{color: "#2F48C1",}} /> 是`无线模式切换键`，用于在 BLE 和 2G4 模式间切换，详见 [切换模式](/docs-elysium/tri-mode/mode-switch#select-mode-via-keys) 页面。

<Icon icon="fa-solid fa-square" style={{color: "#D38322",}} /> 是 `RGB 矩阵调控键`，调节 RGB 灯光的开关、灯效、亮度等参数，详见 [RGB 矩阵](/docs-elysium/lighting/rgb-matrix#keycode-position) 页面。

<Icon icon="fa-solid fa-square" style={{color: "#C12960",}} /> 是`复位键`，用于无线模式工作不正常时复位，详见 [复位](/docs-elysium/miscellaneous/reset) 页面。

<Icon icon="fa-solid fa-square" style={{color: "#AA36C6",}} /> 是`解除配对键`，用于在 BLE 模式下遗忘已配对的主机，详见 [BLE](/docs-elysium/tri-mode/ble#unpairing) 页面。
